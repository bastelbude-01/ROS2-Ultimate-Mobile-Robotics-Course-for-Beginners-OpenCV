...
class camera_sub(Node):
    ...
    def __init__(self):
        super().__init__('line_runner_node')
        self.camera_sub = self.create_subscription(Image,'/udemy_bot_camera/image_raw',self.camera_cb,10)
    ...
    def camera_cb(self, data):
        ...
        frame = frame[290:479,165:475] # First is Y and sencond is X
        ...
        if ( error < 20):
            self.vel_msg.angular.z = -0.4 # rechts drehen
        if ( error < 50):
            self.vel_msg.angular.z = -0.7 
        if ( error < 100):
            self.vel_msg.angular.z = -1.2 
        if ( error < -20):
            self.vel_msg.angular.z = 0.4 # links drehen
        if ( error < -50):
            self.vel_msg.angular.z = 0.7
        if ( error < -100):
            self.vel_msg.angular.z = 1.2
        ...
        cv2.imshow('Fpv from Udemy_Bot',frame)
